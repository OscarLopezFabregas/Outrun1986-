...
{
  LOG ("Loading scene");
  Sprites = App->textures->Load ("resources/Sprites/BeachTrack.png");
  background = App->textures->Load ("resources/Sprites/BeachTrackBackg2.png");
  GUI = App->textures->Load ("resources/Sprites/GUI.png");
...
ModuleScene::TimeManager ()
{
  ...
  if (miliSecondsLap == 60)
    miliSecondsLap = 0;
}


void
ModuleScene::PrintTrack ()
{
  ...
  for (int n = startPos; n < startPos + 300; n++)
	...
	n++;
      ...
      App->renderer->BlitPolygon (grass, 0, (short) p.Y, (short) p.width, 0,
				  (short) l.Y, (short) l.width);
      App->renderer->BlitPolygon (rumble, (short) p.X, (short) p.Y,
				  (short) (p.W * 1.2), (short) l.X,
				  (short) l.Y, (short) (l.W * 1.2));
      App->renderer->BlitPolygon (sideline, (short) p.X, (short) p.Y,
				  (short) (p.W * 1.07), (short) l.X,
				  (short) l.Y, (short) (l.W * 1.07));
      App->renderer->BlitPolygon (road, (short) p.X, (short) p.Y, (short) p.W,
				  (short) l.X, (short) l.Y, (short) l.W);
      App->renderer->BlitPolygon (sideline, (short) p.X, (short) p.Y,
				  (short) (p.W * 0.40), (short) l.X,
				  (short) l.Y, (short) (l.W * 0.40));
      App->renderer->BlitPolygon (road, (short) p.X, (short) p.Y,
				  (short) p.W * 0.33, (short) l.X,
				  (short) l.Y, (short) l.W * 0.33);
...
void
ModuleScene::PrintGUI ()
{
  App->renderer->Blit (GUI, 30, 30, &guiTime, 0.0f, guiTime.w / 2,
		       guiTime.h / 2);
  App->renderer->Blit (GUI, 180, 30, &guiScore, 0.0f, guiScore.w / 2,
		       guiScore.h / 2);
  App->renderer->Blit (GUI, 420, 30, &guiLap, 0.0f, guiLap.w / 2,
		       guiLap.h / 2);
  App->renderer->Blit (GUI, 120, SCREEN_HEIGHT - 50, &guiKmh, 0.0f,
		       guiKmh.w / 2, guiKmh.h / 2);
  App->renderer->Blit (GUI, 470, SCREEN_HEIGHT - 50, &guiStage, 0.0f,
		       guiStage.w / 2, guiStage.h / 2);
  App->renderer->Blit (GUI, 600, SCREEN_HEIGHT - 50, &guiMap, 0.0f,
		       guiMap.w / 2, guiMap.h / 2);
  if (secondsToQuit > 0)
    {
      App->fontManager->PrintCharacter (App->fontManager->yellowFont, 110, 35,
					std::to_string (secondsToQuit));

    }
  App->fontManager->PrintCharacter (App->fontManager->blueFont, 290, 35,
				    std::to_string (score));
  if (minuteLap < 10)
    App->fontManager->PrintCharacter (App->fontManager->greenFont, 500, 35,
				      "0");
  if (secondsLap < 10)
    {
      App->fontManager->PrintCharacter (App->fontManager->greenFont, 560, 35,
					"0");
      App->fontManager->PrintCharacter (App->fontManager->greenFont, 582, 35,
					std::to_string (secondsLap));

    }
  else
    App->fontManager->PrintCharacter (App->fontManager->greenFont, 560, 35,
				      std::to_string (secondsLap));
  App->fontManager->PrintCharacter (App->fontManager->greenFont, 522, 35,
				    std::to_string (minuteLap));
  App->fontManager->PrintCharacter (App->fontManager->greenFont, 565,
				    SCREEN_HEIGHT - 47, "1");
  App->fontManager->PrintSpeed (App->fontManager->speedFont, 60,
				SCREEN_HEIGHT - 50, std::to_string (speed));


}

void
ModuleScene::AnimationPlayer ()
{
  for (int n = startPos; n < startPos + 2; n++)
    {
  ...
  App->renderer->Blit (App->player->graphics,
		       (SCREEN_WIDTH -
			2 * App->player->carState->GetCurrentFrame ().w) / 2,
		       (SCREEN_HEIGHT -
			2 * App->player->carState->GetCurrentFrame ().h) - 5,
		       &(App->player->carState->GetCurrentFrame ()), 1.0f,
		       2 * App->player->carState->GetCurrentFrame ().w,
		       2 * App->player->carState->GetCurrentFrame ().h);
