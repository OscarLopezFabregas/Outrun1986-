...
ModuleFontManager::ModuleFontManager(bool start_enabled)
...
bool ModuleFontManager::Start() {
  ...
  yellowFont = App->textures->Load("resources/Fonts/yellowFont.png");
  greenFont = App->textures->Load("resources/Fonts/greenFont2.png");
  blueFont = App->textures->Load("resources/Fonts/blueFont2.png");
  speedFont = App->textures->Load("resources/Fonts/Speed.png");

  return ret;
}

bool ModuleFontManager::CleanUp() {
  LOG("Cleaning fonts") App->textures->Unload(yellowFont);
...
void ModuleFontManager::PrintCharacter(SDL_Texture *font, int positionX,
                                       int positionY, std::string textToPrint) {
  ...
  for (char character : textToPrint) {
    ...
    if (&charPointer != nullptr) {
      ...
      if (character == '.')
        printRect.y += 5;
      if (SDL_RenderCopy(App->renderer->renderer, fontToPrint, &charPointer,
                         &printRect) != 0) {
...
void ModuleFontManager::PrintSpeed(SDL_Texture *font, int positionX,
                                   int positionY, std::string textToPrint) {
  SDL_Texture *fontToPrint = font;
  SDL_Rect printRect;
  printRect.x = positionX;
